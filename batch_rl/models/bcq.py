...
class BCQ(nn.Module):
    ...
    def update(self, data):
        generator_loss, state = self._compute_generator_loss(
    ...
    def _compute_generator_loss(self, data, action):
        ...
        return generator_loss, state

    def _compute_perturbator_loss(self, state):
        sampled_actions = self.generator.decode(state)
        perturbed_actions = self.perturbator(state, sampled_actions)
        perturbator_loss = -self.critic1(state, perturbed_actions).mean()
        return perturbator_loss, perturbed_actions
